...
path.append(".")
from src.graph_cut.exp_potential import ExpGraphCut

import pathlib


class MyGhraphCut(ExpGraphCut):
    """final graph cut class with uno potential defined
    """
    def __init__(self, image: Image, lambd: float) -> None:
        super().__init__(image, lambd)

        # learn "water pixel"
        self._water_pixel_ = np.mean(self.image_[250:300, 480:500], axis=(0, 1))
...
def main():
    ...
    reduce_factor = 8
    image = Image.open("data/archive/Image/17.jpg").reduce(reduce_factor)
    ...
    cur_results_path = f"experiments/graph_cut/experiment_photo_17_exp/results_init"
    ...
    real_seg_mask = np.array(Image.open("data/archive/Mask/17.png").reduce(reduce_factor), dtype=bool)
