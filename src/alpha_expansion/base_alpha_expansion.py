...
class BaseAlphaExpansion(ABC):
    def __init__(self, 
                 image: Image,
                 num_classes: int,
                 init_seg_matrix: np.ndarray = None,
    ) -> None:
        self.image_ = np.array(image, dtype=np.uint8)
        self.num_classes_ = num_classes

        # set initial segmentation matrix
        if init_seg_matrix is None:
            image_size = self.image_.shape[0:2]
            self._seg_matrix_ = np.random.randint(0, self.num_classes_, size=image_size, dtype=np.int32)
        else:
            self._seg_matrix_ = init_seg_matrix.copy()

        # all segmentation matrices during algorithm iterations
    ...
    @property
    def seg_matrix_seq(self) -> np.ndarray:
        return self._seg_matrix_seq_
    ...
    def alpha_expansion(self, max_iter: int = 10) -> None:
        ...
        # debug
        print("Segmentation's done")

    
    def _reset_results(self):
        self._seg_matrix_seq_ = self._seg_matrix_seq_[0:1]
        self._seg_matrix_ = self._seg_matrix_seq_[0].copy()
        self._energies_ = []
    ...
    def _alpha_exp_iter(self, cur_alpha: int) -> bool:
        ...
        for node in right_partition:
            if node != "t":
                i, j = node
                self._seg_matrix_[i][j] = cur_alpha

        # save new collage matrix and energy
        self._seg_matrix_seq_.append(self._seg_matrix_.copy())
        self._energies_.append(min_energy)

        # debug
        print(f"Segmentation matrix update: Okey; Time = {time() - time_past}")
        time_past = time()
